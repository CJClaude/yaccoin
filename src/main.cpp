...
uint256 hashGenesisBlock("0x2d0a71a1dc8742e5644d100aa28e31a99dccedf1be446d0c30b1ab85f2a3f1a5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // YACCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "YACCoin Signed Message:\n";
...
{
    // YACCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // YACCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1.5 * 60 * 1000; // YACCoin: e.g.  every 1000 blocks
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // YACCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // YACCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1390587883;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084660174;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // YACCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YACCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YACCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static YACCoinMiner(CWallet *pwallet)
{
    printf("YACCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("yaccoin-miner");
    ...
    try { while (true)  {
        ...
        printf("Running YACCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("YACCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YACCoinMiner, pwallet));
}
